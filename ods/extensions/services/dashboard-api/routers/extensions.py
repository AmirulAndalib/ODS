...
def _curated_library_recipe(recipe_dir: Path) -> bool:
    ...
    The shared compose policy's upstream.json classification, which
    _staged_library_extension and the compose resolver (_library_recipe_trusted)
    also use: an imported GitHub recipe (origin github-proposal) or an
    unclassifiable marker never inherits curated-library privileges.
    """
    return _compose_policy_library_origin(recipe_dir) == "curated"

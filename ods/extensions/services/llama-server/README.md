...
| Intel Arc / SYCL (`docker-compose.intel.yml`, `docker-compose.arc.yml`) | b9014 | none (not measured on Intel) |
| Apple Docker (`docker-compose.apple.yml`) | b9014 | none |
| Native Windows llama-server (Vulkan fallback) | b9014 (installs from before this pin keep b8248) | none (not measured on Windows) |
...
| Intel Arc local build (`images/llama-sycl`) | tag `b9014`, commit `d4b0c22f9e67f0295e91dc1ab4f17c0fb2557fa4` |
...
- Native Windows passes `LLAMA_REASONING` as `--reasoning` when the installed `llama-server --help` lists it (b9014), and as `--reasoning-format` otherwise (b8248). b9014 defaults `--reasoning` to `auto`, and `--reasoning-format none` alone returns the reasoning inside the reply.

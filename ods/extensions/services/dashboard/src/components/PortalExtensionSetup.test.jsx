...
test('shows the declared format and the API format refusal without the value', async () => {
  ...
  formatted.steps[0].configuration[0].format = {hint: '64 lowercase hexadecimal characters (0-9, a-f)'}
  const message = 'DEMO_PASSWORD must be 64 lowercase hexadecimal characters (0-9, a-f). Nothing was saved.'
  const fetcher = vi.fn().mockResolvedValueOnce(response(formatted)).mockResolvedValueOnce({
    ok: false, status: 422, json: async () => ({detail: {code: 'invalid_configuration', service_id: 'demo', message,
      invalid_configuration: [{key: 'DEMO_PASSWORD', expected: '64 lowercase hexadecimal characters (0-9, a-f)'}]}}),
  })
  vi.stubGlobal('fetch', fetcher)
  const resume = vi.fn()
  render(<PortalExtensionSetup command="/extensions @demo" onConfigured={resume}/>)
  expect(await screen.findByText('Format: 64 lowercase hexadecimal characters (0-9, a-f)')).toBeInTheDocument()

...
test('Portuguese owner wording is extracted the same way', () => {
  assert.deepEqual(extractRequestedLiterals(PT_CREATE).map(literal => ({...literal})), [
    ...
    {text: 'Jazz ao amanhecer', match: 'item', targets: ['heading']},
    {text: 'Caminhada das lanternas', match: 'item', targets: ['heading']},
    {text: 'Show esgotado da meia-noite', match: 'item', targets: ['heading']},
  ]);
